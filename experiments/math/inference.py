...
from experiments.math.data_utils import prepare_math_dataset
...
def build_prompt(question: str) -> str:
	return (
		"Task: Solve the following math problem. You may reason briefly, then give ONLY the final numeric answer.\n"
		"Rules:\n"
...
def build_5shot_prompt(question: str, shots: list) -> str:
	prefix = (
		"Task: Solve math problems. For each example, show concise Solution and finish with 'Final Answer: <number>'.\n"
		"Rules:\n"
		"- Do NOT include units or extra words\n"
...
def predict_and_score(model, tokenizer, dataset_rows, max_length: int, max_gen: int):
	...
	for ex in tqdm(dataset_rows):
		prompt = ex.get("prompt") or build_prompt(ex.get("question", ""))
...
def main():
	...
	parser.add_argument('--mode', type=str, choices=['ada', 'fix', 'base', 'sv', 'headkv', 'snapkv'], default='base')
	...
	parser.add_argument('--out_dir', type=str, default='./experiments/math/pred')
	...
	if args.compress_args_path:
		...
		if args.mode in ['ada', 'sv', 'headkv', 'snapkv']:
	...
	if args.compress_args_path:
		if args.mode == 'sv':
			model.model.config.given_adaptive_size = set_cc_size(compress_args['base_capacity'], compress_args['window_size'], 'qwen3', 'average', args.sv_beta)
		elif args.mode == 'headkv':
			model.model.config.given_adaptive_size = set_headkv_size(compress_args['base_capacity'], compress_args['window_size'], 'qwen', args.headkv_beta)
	...
	paths = prepare_math_dataset('./adaptive_kv/assets/datasets', valid_size=500, test_size=1000)
