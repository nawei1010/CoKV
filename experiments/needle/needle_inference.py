...
from adaptive_kv.monkeypatch.utils import set_cc_size,set_snapkv_size,set_headkv_size
...
class LLMNeedleHaystackTester:
    ...
    def __init__(self,
        ...
        if(self.model_provider in ["Qwen3", "LLaMA3", "Mistral"]):
            ...
            compress_args = json.load(open(os.path.join(config_dir, args.compress_args_path), "r"))
        ...
        else:raise ValueError("model_provider must be 'Qwen3', 'LLaMA3' or 'Mistral'")
    ...
    def generate_prompt(self, context):
        ...
        if self.model_provider == "Qwen3":
            messages = [
                {"role": "user", "content": f"This is a very long story book: <book> {context} </book>.\nBased on the content of the book, Keep your response short and direct.Question: {self.retrieval_question}\n Answer:"}
            ]
            text = self.enc.apply_chat_template(messages, tokenize=False, add_generation_prompt=True, enable_thinking=False)
            return text
    ...
    def evaluate_and_log(self, context_length, depth_percent):
        ...
        if(self.model_provider in ["Qwen3", "LLaMA3", "Mistral"]):
            ...
            if self.model_provider == "Qwen3":
                model_inputs = self.enc([prompt], return_tensors="pt").to(self.model_to_test.device)
    ...
    def read_context_files(self):
        ...
        if not files:
            raise FileNotFoundError(f"No .txt files found in {self.haystack_dir}. Please set --haystack_dir correctly.")
    ...
    def start_test(self, args):
        ...
        avg_score, results = self.run_test(args)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--haystack_dir', type=str, default=None, help='Directory containing .txt haystack files')
    parser.add_argument('--scope', type=int, default=None, help='Scope identifier to include in output filename')
    parser.add_argument('--step', type=int, default=1000)
    parser.add_argument('--mode', type=str, choices=['ada', 'fix', 'base','cokv','headkv','snapkv'], help="Ada mode, fix mode or normal")
    parser.add_argument('--cokv_alpha',type=int,help="a number")
    parser.add_argument('--headkv_beta',type=float,help="a number")
    ...
    ht = LLMNeedleHaystackTester(model_name=args.model_name, 
                            ...
                            haystack_dir=args.haystack_dir if args.haystack_dir is not None else "your/haystack/path",
    ...
    if args.mode == 'cokv':
        ht.change_given_adaptive_size(set_cc_size(128,8,args.model, 'average', args.cokv_alpha))
    elif args.mode == 'headkv':
        ht.change_given_adaptive_size(set_headkv_size(128,8,args.model,args.headkv_beta))
    elif args.mode =='snapkv':
        ht.change_given_adaptive_size(set_snapkv_size(128,8))

    average_acc, acc_list = ht.start_test(args)

    # Build concise JSON of results by context length and depth percent
    used_context_lengths = [int(x) for x in ht.context_lengths if x >= args.s_len and x <= args.e_len]
    depth_percents = [float(d) for d in ht.document_depth_percents]

    # Infer scope from compress_args_path if not provided
    scope_val = args.scope
    if scope_val is None and args.compress_args_path is not None:
        basename = os.path.basename(args.compress_args_path)
        m = re.match(r"c(\d+)_", basename)
        if m:
            try:
                scope_val = int(m.group(1))
            except Exception:
                scope_val = None
    ...
    out_name = f"results_{args.mode}_scope_{scope_val}_{args.cokv_alpha}.json"
    out_path = os.path.join(out_dir, out_name)

    minimal_results = {
        'context_lengths': used_context_lengths,
        'depth_percents': depth_percents,
        'scores': acc_list,
    }

    with open(out_path, 'w') as f:
        json.dump(minimal_results, f, ensure_ascii=False)
