...
def set_headkv_size(base_capability, window_size, model, beta):
    if model == 'llama':
        path = f'{project_path}/adaptive_kv/headkv/head_score/Meta-Llama-3-8B-Instruct_retrieval_reasoning_heads.json'
    elif model == 'mistral':
        path = f'{project_path}/adaptive_kv/headkv/head_score/Mistral-7B-Instruct-v0.2_retrieval_reasoning_heads.json'
    ...
    with open(path, 'r') as file:
        ...
        cache_size = torch.zeros(32,8)
        for i in range(32):
            for j in range(8):
                for k in range(4):
                    cache_size[i][j] += head_capacity[i][j*4+k]
                cache_size[i][j] = cache_size[i][j]/4

    return np.floor(cache_size)


def set_cokv_size(base_capability, window_size, model, dataset, k_th):
    base_capability = base_capability-window_size
    sum_capability = base_capability*32*8
    cache_size = torch.zeros(32,8,dtype=torch.int32)

    cc_left = torch.zeros(257)
    count_left = torch.zeros(257)
    Avg_cc = torch.zeros(257)

    if model == 'llama':
        files = [f'cc_{dataset}_32',f'cc_{dataset}_64',f'cc_{dataset}_96',f'cc_{dataset}_128']
    else:
        files = [f'cc_mistral_{dataset}_32',f'cc_mistral_{dataset}_64',f'cc_mistral_{dataset}_96',f'cc_mistral_{dataset}_128']
    for file in files:
        data = np.load(f'{project_path}/adaptive_kv/cc_scores/cc_{model}/{file}.npy', allow_pickle=True).item()
...
def set_snapkv_size(base_capability, window_size):
    base_capability = base_capability-window_size
    cache_size = torch.ones(32,8,dtype=torch.int32)*base_capability
    return np.floor(cache_size)

def cokv_mask(model, dataset, flag, k_th):
    cache_size = torch.ones(32,8,dtype=torch.int32)*8000
    ...
    if flag == 'top':
        border = search_kth_smallest(CC, 256-k_th)
        for i in range(32):
            for j in range(8):
                if CC[i][j] >= border:
                    cache_size[i][j] = 0
...
def headkv_mask(model, flag, k_th):
    cache_size = torch.ones(32,8,dtype=torch.int32)*8000
    if model == 'llama':
        path = f'{project_path}/adaptive_kv/headkv/head_score/Meta-Llama-3-8B-Instruct_retrieval_reasoning_heads.json'
    elif model == 'mistral':
    ...
    with open(path, 'r') as file:
        ...
        num_attention_heads = 32
        total_attention = head_score_list.reshape(num_hidden_layers, num_attention_heads)
        group_score = torch.zeros(32,8)
        for i in range(32):
            for j in range(8):
                for k in range(4):
                    group_score[i][j] += total_attention[i][j*4+k]
        ...
        if flag == 'top':
            border = search_kth_smallest(group_score, 256-k_th)
            for i in range(32):
                for j in range(8):
...
def random_mask(del_group):
    cache_size = torch.ones(32,8,dtype=torch.int32)*8000
    numbers = random.sample(range(256), del_group)
    for num in numbers:
